...
Function CreateDeskIcon
	...
	${If} $2 == 1 ;win7
		...
		${AndIf} $3 != 0
			...
			IfFileExists "$3\TaskBar\启动 Internet Explorer 浏览器.lnk" 0 +8
				...
				System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::Pin2TaskbarWin7(t '$3\TaskBar\启动 Internet Explorer 浏览器.lnk', b false) "
			...
			${For} $R1 2 10
				...
				IfFileExists "$3\TaskBar\启动 Internet Explorer 浏览器 ($R1).lnk" 0 +2
					System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::Pin2TaskbarWin7(t '$3\TaskBar\启动 Internet Explorer 浏览器 ($R1).lnk', b false) "
			${Next}
			SetOutPath "$PROGRAMFILES\Internet Explorer"
			...
			IfFileExists "$3\StartMenu\Internet.lnk" 0 +7
				...
				Sleep 1
				Sleep 1
			IfFileExists "$3\StartMenu\启动 Internet Explorer 浏览器.lnk" 0 +7
				${If} $Bool_IsUpdate == 0
					WriteRegStr HKCU "SOFTWARE\iexplorer" "STARTMENU" "1"
					CopyFiles /silent "$3\StartMenu\启动 Internet Explorer 浏览器.lnk" "$0\IECFG\lnkbak\STARTMENU_启动 Internet Explorer 浏览器.lnk"
				${EndIf}
				System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::Pin2StartMenuWin7(t '$3\StartMenu\启动 Internet Explorer 浏览器.lnk', b false) "
				Sleep 1000
				Sleep 1
				Sleep 1
			${For} $R1 2 10
				...
				IfFileExists "$3\StartMenu\启动 Internet Explorer 浏览器 ($R1).lnk" 0 +2
					System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::Pin2StartMenuWin7(t '$3\StartMenu\启动 Internet Explorer 浏览器 ($R1).lnk', b false) "
			${Next}
			CreateShortCut "$SMPROGRAMS\Internet Explorer.lnk" "$0\${PRODUCT_NAME}\program\iexplore.exe" "/sstartfrom startmenuprograms" "" "" "" "" "启动 Internet Explorer 浏览器"
	...
	${Else}
		...
		IfFileExists "$QUICKLAUNCH\启动 Internet Explorer 浏览器.lnk" 0 QUICKLAUNCHOK2
			...
			System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::DeleteFileEx(t '$QUICKLAUNCH\启动 Internet Explorer 浏览器.lnk') "
			QUICKLAUNCHOK2:
		...
		IfFileExists "$SMPROGRAMS\启动 Internet Explorer 浏览器.lnk" 0 STARTMENUOK3
			...
			System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::DeleteFileEx(t '$SMPROGRAMS\启动 Internet Explorer 浏览器.lnk') "
		STARTMENUOK3:
		;删除开始菜单下的ie图标
		...
		IfFileExists "$STARTMENU\启动 Internet Explorer 浏览器.lnk" 0 STARTMENUOK33
			...
			System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::DeleteFileEx(t '$STARTMENU\启动 Internet Explorer 浏览器.lnk') "
		STARTMENUOK33:
	...
	${If} $2 == 1
		...
		${AndIf} $3 != 0
			...
			${If} $R6 == "1" 
				...
				CopyFiles /silent "$0\IECFG\lnkbak\QUICKLAUNCH_启动 Internet Explorer 浏览器.lnk" "$TEMP\启动 Internet Explorer 浏览器.lnk"
				ExecShell taskbarpin "$TEMP\启动 Internet Explorer 浏览器.lnk"
				Goto TASKRERSTOREOK
			...
			${If} $R8 == "1" 
				...
				CopyFiles /silent "$0\IECFG\lnkbak\STARTMENU_启动 Internet Explorer 浏览器.lnk" "$TEMP\启动 Internet Explorer 浏览器.lnk"
				StrCpy $R0 "$TEMP\启动 Internet Explorer 浏览器.lnk"
				System::Call "$TEMP\${PRODUCT_NAME}\YBSetUpHelper::RefleshIcon(t '$R0')"
				Sleep 1000
				ExecShell startpin "$TEMP\启动 Internet Explorer 浏览器.lnk"
				StartMenuRestoreOK:
			${EndIf}
		${EndIf}
	${Else}
